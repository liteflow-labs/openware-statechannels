//
// Flow 5
// 3 participants, 3 states. each participants signs a different state. challenge to finalize the channel, but reopen the channel by sending a respond
//
...
async function main() {
  ...
  await (async () => {
    ...
    const responseSignature = await signState(newState, responder.privateKey)
      .signature
